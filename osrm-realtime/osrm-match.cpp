...
int main(int argc, const char *argv[])
{
    ...
    if (status == Status::Ok)
    {
        ...
        const auto totalDistance = match.values["distance"].get<json::Number>().value;
        const auto totalDuration = match.values["duration"].get<json::Number>().value;

        auto &legs = match.values["legs"].get<json::Array>();
